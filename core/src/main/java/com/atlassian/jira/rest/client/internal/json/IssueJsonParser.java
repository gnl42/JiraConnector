...
//        final String name = jsonObject.getString("name");
	...
	public Issue parse(JSONObject s) throws JSONException {
		...
		if (isJira5x0OrNewer) {
			final JSONObject commentsJson = s.getJSONObject(FIELDS).getJSONObject(IssueFieldId.COMMENT_FIELD.id);
			...
			final Collection<Comment> commentsTmp = parseOptionalArray(
					shouldUseNestedValueAttribute, s, new JsonWeakParserForJsonObject<Comment>(commentJsonParser), FIELDS, IssueFieldId.COMMENT_FIELD.id);
			comments = commentsTmp != null ? commentsTmp : Lists.<Comment>newArrayList();
		}

		final String summary = getFieldStringValue(s, IssueFieldId.SUMMARY_FIELD.id);
		final String description = getOptionalFieldStringUnisex(shouldUseNestedValueAttribute, s, IssueFieldId.DESCRIPTION_FIELD.id);

		final Collection<Attachment> attachments = parseOptionalArray(shouldUseNestedValueAttribute, s, new JsonWeakParserForJsonObject<Attachment>(attachmentJsonParser), FIELDS, IssueFieldId.ATTACHMENT_FIELD.id);
		final Collection<Field> fields = isJira5x0OrNewer ? parseFieldsJira5x0(s) : parseFields(s.getJSONObject(FIELDS));

		final BasicIssueType issueType = issueTypeJsonParser.parse(getFieldUnisex(s, IssueFieldId.ISSUE_TYPE_FIELD.id));
		final DateTime creationDate = JsonParseUtil.parseDateTime(getFieldStringUnisex(s, IssueFieldId.CREATED_FIELD.id));
		final DateTime updateDate = JsonParseUtil.parseDateTime(getFieldStringUnisex(s, IssueFieldId.UPDATED_FIELD.id));

		final String dueDateString = getOptionalFieldStringUnisex(shouldUseNestedValueAttribute, s, IssueFieldId.DUE_DATE_FIELD.id);
		final DateTime dueDate = dueDateString == null ? null : JsonParseUtil.parseDateTimeOrDate(dueDateString);

		final BasicPriority priority = getOptionalField(shouldUseNestedValueAttribute, s, IssueFieldId.PRIORITY_FIELD.id, priorityJsonParser);
		final BasicResolution resolution = getOptionalField(shouldUseNestedValueAttribute, s, IssueFieldId.RESOLUTION_FIELD.id, resolutionJsonParser);
		final User assignee = getOptionalField(shouldUseNestedValueAttribute, s, IssueFieldId.ASSIGNEE_FIELD.id, userJsonParser);
		final User reporter = getOptionalField(shouldUseNestedValueAttribute, s, IssueFieldId.REPORTER_FIELD.id, userJsonParser);

		final BasicProject project = projectJsonParser.parse(getFieldUnisex(s, IssueFieldId.PROJECT_FIELD.id));
		final Collection<IssueLink> issueLinks;
		if (isJira5x0OrNewer) {
			issueLinks = parseOptionalArray(shouldUseNestedValueAttribute, s,
					new JsonWeakParserForJsonObject<IssueLink>(issueLinkJsonParserV5), FIELDS, IssueFieldId.LINKS_FIELD.id);
		} else {
			issueLinks = parseOptionalArray(shouldUseNestedValueAttribute, s,
					new JsonWeakParserForJsonObject<IssueLink>(issueLinkJsonParser), FIELDS, IssueFieldId.LINKS_PRE_5_0_FIELD.id);
		}

		Collection<Subtask> subtasks = null;
		if (isJira5x0OrNewer) {
			subtasks = parseOptionalArray(shouldUseNestedValueAttribute, s, new JsonWeakParserForJsonObject<Subtask>(subtaskJsonParser), FIELDS, IssueFieldId.SUBTASKS_FIELD.id);
		}

		final BasicVotes votes = getOptionalField(shouldUseNestedValueAttribute, s, IssueFieldId.VOTES_FIELD.id, votesJsonParser);
		final BasicStatus status = statusJsonParser.parse(getFieldUnisex(s, IssueFieldId.STATUS_FIELD.id));

		final Collection<Version> fixVersions = parseOptionalArray(shouldUseNestedValueAttribute, s, new JsonWeakParserForJsonObject<Version>(versionJsonParser), FIELDS, IssueFieldId.FIX_VERSIONS_FIELD.id);
		final Collection<Version> affectedVersions = parseOptionalArray(shouldUseNestedValueAttribute, s, new JsonWeakParserForJsonObject<Version>(versionJsonParser), FIELDS, IssueFieldId.AFFECTS_VERSIONS_FIELD.id);
		final Collection<BasicComponent> components = parseOptionalArray(shouldUseNestedValueAttribute, s, new JsonWeakParserForJsonObject<BasicComponent>(basicComponentJsonParser), FIELDS, IssueFieldId.COMPONENTS_FIELD.id);
		...
		if (s.has(IssueFieldId.TRANSITIONS_FIELD.id)) {
			...
			transitionsUriString = getOptionalFieldStringUnisex(shouldUseNestedValueAttribute, s, IssueFieldId.TRANSITIONS_FIELD.id);
		}
		final URI transitionsUri = parseTransisionsUri(transitionsUriString, selfUri);

		if (isJira5x0OrNewer) {
			if (JsonParseUtil.getNestedOptionalObject(s, FIELDS, IssueFieldId.WORKLOG_FIELD.id) != null) {
				worklogs = parseOptionalArray(shouldUseNestedValueAttribute, s,
						...
						FIELDS, IssueFieldId.WORKLOG_FIELD.id, IssueFieldId.WORKLOGS_FIELD.id);
			} else {
				worklogs = Collections.emptyList();
			}
		} else {
			worklogs = parseOptionalArray(shouldUseNestedValueAttribute, s, new JsonWeakParserForJsonObject<Worklog>(worklogJsonParser), FIELDS, IssueFieldId.WORKLOG_FIELD.id);
		}

		final BasicWatchers watchers = getOptionalField(shouldUseNestedValueAttribute, s,
				isJira5x0OrNewer ? IssueFieldId.WATCHER_FIELD.id : IssueFieldId.WATCHER_PRE_5_0_FIELD.id, watchersJsonParser);
		final TimeTracking timeTracking = getOptionalField(shouldUseNestedValueAttribute, s, IssueFieldId.TIMETRACKING_FIELD.id,
		...
		final Set<String> labels = Sets.newHashSet(parseOptionalArrayNotNullable(shouldUseNestedValueAttribute, s,
				jsonWeakParserForString, FIELDS, IssueFieldId.LABELS_FIELD.id));
