...
//        BambooCorePlugin.getRepositoryConnector().getClientManager().setTaskRepositoryLocationFactory(new TaskRepositoryLocationUiFactory());
    }
    ...
    @Override
    public IWizard getNewTaskWizard(final TaskRepository taskRepository, final ITaskMapping selection) {
        return null;
    }
    ...
    @Override
    public String getTaskKindLabel(final ITask task) {
        return "Build Plan";
    }

    @Override
    public String getAccountCreationUrl(final TaskRepository taskRepository) {
        return taskRepository.getRepositoryUrl() + "/signupUser!default.action"; //$NON-NLS-1$
    }

    @Override
    public String getAccountManagementUrl(final TaskRepository taskRepository) {
