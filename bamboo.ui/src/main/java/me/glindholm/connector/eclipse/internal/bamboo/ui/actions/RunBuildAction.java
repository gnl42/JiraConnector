...
import me.glindholm.connector.eclipse.internal.bamboo.ui.operations.RunBuildJob;
...
public class RunBuildAction extends EclipseBambooBuildSelectionListenerAction {
    ...
    /**
     *
     * @param action optional action to run (may be null)
     */
    public RunBuildAction(@Nullable final Action action) {
        super(null);
        this.action = action;
        initialize();
    }
    ...
    @Override
    void onRun(final EclipseBambooBuild eclipseBambooBuild) {
            ...
            public void done(final IJobChangeEvent event) {
                ...
                if (event.getResult().getCode() == IStatus.ERROR) {
                    Display.getDefault().syncExec(new Runnable() {
                        @Override
                        public void run() {
                            MessageDialog.openError(null, getText(), "Running build " + build.getPlanKey() + " failed. See Error Log for details.");
                        }
                    });
                }
            }
    ...
    @Override
    boolean onUpdateSelection(final EclipseBambooBuild eclipseBambooBuild) {
