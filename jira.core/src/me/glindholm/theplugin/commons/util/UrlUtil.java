...
/**
 * Created by IntelliJ IDEA. User: Jacek Date: 2008-02-27 Time: 14:20:49 To change this template use
...
public abstract class UrlUtil {
    ...
    public static void validateUrl(final String urlString) throws MalformedURLException {
    ...
    /**
     * Removes the junction part of the urlPath and serverUrl from the urlPath and returns modified
     * urlPath
     *
     * @param urlPath
     * @param serverUrl
     * @return
     */
    public static String adjustUrlPath(String urlPath, final String serverUrl) {
        final String[] serverTokens = serverUrl.split("/");
        final String[] pathTokens = urlPath.split("/");

        if (serverTokens.length > 0 && pathTokens.length > 0) {
            if (serverTokens[serverTokens.length - 1].equals(pathTokens[0])) {
                urlPath = urlPath.substring(pathTokens[0].length());
            }
        }
        return urlPath;
    }
