...
/**
 * Communication stub for lightweight XML based APIs. This method should be tread-safe (at least it
 * is used in this manner), however I think there are still some issues with thread-safety here
 * [wseliga]. E.g. as Server is not immutable then this may be the cause of races
 */
public abstract class AbstractHttpSession {
    ...
    // TODO: replace this with a proper cache to ensure automatic purging. Responses can get quite
    // large.
     ...
     * @throws me.glindholm.theplugin.commons.remoteapi.RemoteApiMalformedUrlException for malformed url
     */
    ...
    /**
     * PayPal This method should be use to fetch standard non-XML text resources (like Bamboo build
     * logs), when there is no intention to parse them by XML and you want to respect HTTP encoding
     * standards (e.g. ISO-8859-1 if there is no charset info set in the response header. This method
     * does not cache results, nor it supports conditional get.
     *
     * @param urlString URL
     * @return response encoded as String. Encoding respects content type sent by the server in the
     *         response headers
    ...
    /**
     * Use it only for retrieving XML information, as it will ignored content-type charset in response
     * header (if such present)
     *
     * @param urlString URL to retrieve data from
     * @return response as raw bytes (ignoring charset info in response headers). This is OK for XML
     *         parser, as servers supported by us use either encoding info in XML header or use UFT-8
...
//          }
    ...
    private Document retrievePostResponseInternal(final String urlString, final String request, final boolean expectResponse, final int redirectCounter,
        ...
        return retrievePostResponseInternalImpl(urlString, postMethod -> {
        ...
        for (int i = 0; i <= MAX_REDIRECTS; i++) {
            ...
            synchronized (clientLock) {
                ...
                try {
                    ...
                    // throw new RemoteApiException(IOException.class.getSimpleName() + " encountered while posting data
                    // to ["
    ...
    /**
     * This method will connect to server, and return the results of the push You must set Query first,
     * which is the contents of your XML file
     */
