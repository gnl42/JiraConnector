 ...
 *******************************************************************************/
package me.glindholm.connector.eclipse.internal.jira.core.html;
...
public class HTML2TextReader extends SubstitutionTextReader implements AutoCloseable {
     ...
     */
    @Override
    protected String computeSubstitution(final int c) throws IOException {
    ...
    private String html2Text(String html) {
        ...
        if ("/p".equals(html)) { //$NON-NLS-1$
            final boolean inParagraph = fInParagraph;
            fInParagraph = false;
    ...
    private String processPreformattedText(final int c) {
        if (c == '\r' || c == '\n') {
            fCounter++;
        }
        return null;
    ...
    private void unread(final int ch) throws IOException {
        ((PushbackReader) getReader()).unread(ch);
    }

    protected String entity2Text(final String symbol) {
