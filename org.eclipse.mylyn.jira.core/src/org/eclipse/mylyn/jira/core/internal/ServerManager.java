...
import org.eclipse.mylar.jira.core.internal.service.CachedRpcJiraServer;
...
public class ServerManager {
	...
	private Map serverByName = new HashMap();
	...
	protected void stop() {
		for (Iterator iServers = serverByName.values().iterator(); iServers.hasNext();) {
			JiraServer server = (JiraServer) iServers.next();
	...
	public JiraServer getServer(String name) {
		return (JiraServer) serverByName.get(name);
	}

	public JiraServer[] getAllServers() {
		return (JiraServer[]) serverByName.values().toArray(new JiraServer[serverByName.size()]);
	}
	...
	private void fireServerRemoved(JiraServer server) {
		for (Iterator iListeners = listeners.iterator(); iListeners.hasNext();) {
			JiraServerListener listener = (JiraServerListener) iListeners.next();
