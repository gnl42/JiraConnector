...
public class JiraCommentPartCopy extends AbstractTaskEditorPart {
    ...
    private class CommentGroupViewer {
        ...
        public CommentGroupViewer(final CommentGroup commentGroup) {
            this.commentGroup = commentGroup;
        }

        private Composite createCommentViewers(final Composite parent, final FormToolkit toolkit) {
            final List<CommentViewer> viewers = getCommentViewers();
            ...
            for (final CommentViewer commentViewer : viewers) {
                final Control control = commentViewer.createControl(composite, toolkit);
        ...
        public List<CommentViewer> getCommentViewers() {
            ...
            for (final TaskAttribute commentAttribute : commentGroup.getCommentAttributes()) {
                final CommentViewer commentViewer = new CommentViewer(commentAttribute);
        ...
        public boolean isExpanded() {
            ...
            if (commentViewers != null) {
                for (final CommentViewer commentViewer : commentViewers) {
        ...
        public void setRenderedInSubSection(final boolean renderedInSubSection) {
    ...
    private class CommentViewer {
        ...
        public CommentViewer(final TaskAttribute commentAttribute) {
        ...
        private Composite createTitle(final ExpandableComposite commentComposite, final FormToolkit toolkit) {
            // always visible
            final Composite titleComposite = toolkit.createComposite(commentComposite);
            ...
            final ImageHyperlink expandCommentHyperlink = createTitleHyperLink(toolkit, titleComposite, taskComment);
            ...
            final RowLayout buttonCompLayout = new RowLayout();
            ...
            final ReplyToCommentAction replyAction = new ReplyToCommentAction(this, taskComment);
        ...
        private void expandComment(final FormToolkit toolkit, final Composite composite, final boolean expanded) {
            ...
            if (expanded && composite.getData(KEY_EDITOR) == null) {
                ...
                // dispose viewer
                final AbstractAttributeEditor editor = (AbstractAttributeEditor) composite.getData(KEY_EDITOR);
    ...
    private class ReplyToCommentAction extends AbstractReplyToCommentAction implements IMenuCreator {
        ...
        public ReplyToCommentAction(final CommentViewer commentViewer, final ITaskComment taskComment) {
            super(getTaskEditorPage(), taskComment);
    ...
    public JiraCommentPartCopy() {
        commentGroupStrategy = new CommentGroupStrategy() {
            @Override
            protected boolean hasIncomingChanges(final ITaskComment taskComment) {
    ...
    private void collapseAllComments() {
        try {
            ...
            for (final CommentGroupViewer viewer : viewers) {
    ...
    private void expandSection(final FormToolkit toolkit, final Section section) {
        ...
        final List<CommentGroupViewer> viewers = getCommentGroupViewers();
        for (final CommentGroupViewer viewer : viewers) {
            final Control control = viewer.createControl(composite, toolkit);
    ...
    @Override
    protected void fillToolBar(final ToolBarManager barManager) {
        final Action collapseAllAction = new Action("") { //$NON-NLS-1$
            @Override
            public void run() {
                collapseAllComments();
            }
        ...
        final Action expandAllAction = new Action("") { //$NON-NLS-1$
            @Override
            public void run() {
                expandAllComments();
            }
    ...
    public void setCommentGroupStrategy(final CommentGroupStrategy commentGroupStrategy) {
    ...
    private List<CommentGroupViewer> getCommentGroupViewers() {
        ...
        final String currentPersonId = getModel().getTaskRepository().getUserName();
        final List<CommentGroup> commentGroups = getCommentGroupStrategy().groupComments(comments, currentPersonId);

        commentGroupViewers = new ArrayList<>(commentGroups.size());
        if (commentGroups.size() > 0) {
            for (int i = 0; i < commentGroups.size(); i++) {
                final CommentGroupViewer viewer = new CommentGroupViewer(commentGroups.get(i));
                final boolean isLastGroup = i == commentGroups.size() - 1;
    ...
    public CommentViewer selectReveal(final TaskAttribute commentAttribute) {
        if (commentAttribute == null) {
            return null;
        }
        expandAllComments(false);
        expandGroupViewer(commentAttribute);
        final List<CommentGroupViewer> groupViewers = getCommentGroupViewers();
    ...
    private void expandGroupViewer(final TaskAttribute commentAttribute) {
        try {
            expandAllInProgress = true;
