...
public class AsynchronousIssueRestClientTest extends AbstractAsynchronousRestClientTest {
	...
	public void testWatchUnwatch() {
		...
		Assert.assertThat(issueClient.getWatchers(issue1.getWatchers().getSelf()).claim().getUsers(), not(hasItem(USER_ADMIN)));

		issueClient.watch(issue1.getWatchers().getSelf()).claim();
		Assert.assertThat(issueClient.getWatchers(issue1.getWatchers().getSelf()).claim().getUsers(), hasItem(USER_ADMIN));

		issueClient.unwatch(issue1.getWatchers().getSelf()).claim();
		Assert.assertThat(issueClient.getWatchers(issue1.getWatchers().getSelf()).claim().getUsers(), not(hasItem(USER_ADMIN)));

		Assert.assertThat(issueClient.getWatchers(issue1.getWatchers().getSelf()).claim().getUsers(), hasItem(USER1));
		issueClient.removeWatcher(issue1.getWatchers().getSelf(), USER1.getName()).claim();
		Assert.assertThat(issueClient.getWatchers(issue1.getWatchers().getSelf()).claim().getUsers(), not(hasItem(USER1)));
		issueClient.addWatcher(issue1.getWatchers().getSelf(), USER1.getName()).claim();
		Assert.assertThat(issueClient.getWatchers(issue1.getWatchers().getSelf()).claim().getUsers(), hasItem(USER1));
	}
	...
	public void testWatchAlreadyWatched() {
		...
		Assert.assertThat(client.getIssueClient().getWatchers(issue.getWatchers().getSelf()).claim().getUsers(), hasItem(USER1));
	...
	public void testAddWatcherUnauthorized() {
		...
		assertThat(client.getIssueClient().getWatchers(issue1.getWatchers().getSelf()).claim().getUsers(), hasItem(USER1));
	...
	public void testAddAttachment() throws IOException {
		...
		issueClient.addAttachment(issue.getAttachmentsUri(), AsynchronousIssueRestClientTest.class.getResourceAsStream("/attachment-test/transparent-png.png"), filename2).claim();
		...
		assertTrue(
				IOUtils.contentEquals(AsynchronousIssueRestClientTest.class.getResourceAsStream("/attachment-test/transparent-png.png"),
	...
	public void testFetchingUnassignedIssue() {
		...
		assertEquals(USER_ADMIN, client.getIssueClient().getIssue("TST-5").claim().getAssignee());
