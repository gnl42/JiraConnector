...
public final class FileUtility {
	...
	public static List<IResource> getResourcesByFilterRecursive(IResource[] roots, IStateFilter filter) {
		...
		for (IResource root : roots) {
			...
			if (FileUtility.isIgnored(root)) {//FileUtility.isSVNInternals(roots[i])
				continue;
			}

			IP4Resource local = P4Workspace.getWorkspace().getResource(root);
			if (filter.accept(local)) {
				result.add(root);
			}

			if (root instanceof IContainer && filter.allowsRecursion(local)) {
				result.addAll(getResourcesByFilterRecursive(getAllMembers((IContainer) root), filter));
			}
		}
	...
	public static boolean checkForResourcesPresence(IResource[] roots, IStateFilter filter, int depth) {
		final List<IResource> recursiveCheck = MiscUtil.buildArrayList();
	...
	public static IResource[] getAllMembers(IContainer container) {
		...
		if (folder != null && folder instanceof IP4Container) {
			IP4File[] scmResources = ((IP4Container) folder).getAllLocalFiles();
			if (scmResources != null) {
				List<IResource> resources = MiscUtil.buildArrayList();
				for (int i = 0, s = scmResources.length; i < s; ++i) {
					resources.add(scmResources[i].getLocalFileForLocation());
				}
